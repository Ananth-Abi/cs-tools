...
- **`internal/eventbus`** — `Producer` (publish, wait for ack). The consumer side is **no longer in this backend** — it moved to `integrations/csm-activity-stream-service` (join a named consumer group, poll, commit after `Handle` returns — see that service's `Consumer.Run` doc comment for why it deliberately skips a retry/dead-letter policy, unlike csm-notification-service's own consumer).
...
- **`internal/caseevents.Handler`** — the consumer side is **no longer in this backend** — it moved to `integrations/csm-activity-stream-service` along with the SSE stream. This backend now only publishes events; it no longer consumes them.
