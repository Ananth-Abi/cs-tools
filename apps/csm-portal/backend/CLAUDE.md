...
**Notifications (email, Google Chat, and future channels like SMS/voice-Twilio) have moved out of this backend** into `integrations/csm-notification-service`, a standalone Go service invoked over HTTP. This backend no longer constructs or calls any notification client directly — instead, `internal/eventbus`/`internal/events`/`internal/eventpublisher` handle Azure Event Hub's `case-events` topic, which that service (and now the separate `integrations/csm-portal-activity-stream-service`) reacts to.
...
- **`internal/eventbus`** — `Producer` (publish, wait for ack). The consumer side is **no longer in this backend** — it moved to `integrations/csm-portal-activity-stream-service` (join a named consumer group, poll, commit after `Handle` returns — see that service's `Consumer.Run` doc comment for why it deliberately skips a retry/dead-letter policy, unlike csm-notification-service's own consumer).
...
- **`internal/caseevents.Handler`** — the consumer side is **no longer in this backend** — it moved to `integrations/csm-portal-activity-stream-service` along with the SSE stream. This backend now only publishes events; it no longer consumes them.
