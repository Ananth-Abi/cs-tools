...
function renderModal(c: CsmTimeCard) {
  return render(
    <MemoryRouter>
      <TimeCardDetailModal card={c} onClose={vi.fn()} />
    </MemoryRouter>,
  );
}

describe("TimeCardDetailModal", () => {
  it("shows the engineer's work-log comment, sanitized, when the card has one", () => {
    renderModal(card({ workLogComment: "<p>Investigated the reported latency issue.</p>" }));
  ...
  it("strips unsafe markup from the work-log comment before rendering", () => {
    renderModal(
      card({
        workLogComment: '<p>hi<script>window.x=1</script></p><img src=x onerror="window.y=1">',
      }),
    );
