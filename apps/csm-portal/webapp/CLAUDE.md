...
- **Resolving the target**: prefer `location.state.from` (set by whatever list/detail page the caller came from) over a hardcoded fallback path, so Back returns to the exact originating view rather than skipping past it to a generic list — e.g. `const resolvedBackPath = (location.state as { from?: string } | undefined)?.from ?? "/some/hardcoded/default";`. This is the established convention across `CsmProjectDetailPage.tsx`, `CsmCaseDetailPage.tsx`, `ProductVulnerabilitiesTab.tsx`, and the 4 case-type create pages below.
  - **Exceptions** (fixed-destination Back links, deliberately not `state.from`-aware): `CsmAdminLayout.tsx`'s "Back to User management" link always targets the tile-grid index route (`USER_MANAGEMENT_INDEX_PATH`) — it isn't page-history Back, it's a fixed "go up one level in this directory hierarchy" link, and every directory page under it (`/admin/user-management/*`) is only ever reached by clicking a tile on that same grid. `DirectoryMemberPage.tsx` (backing `RoleMembersPage`/`GroupMembersPage`/`TeamMembersPage`) is the same category: `admin/roles/:id` etc. are only ever reached from their own matching list page. `CsmDashboardBuilderEditorPage.tsx`'s "Back to dashboards" is the same again, only ever reached from `CsmDashboardBuilderListPage.tsx`. `DashboardWidgetPreviewPage.tsx` hardcodes Back to `/dashboard` for a different reason — it's deliberately fully URL-driven (bookmarkable/shareable), not state-based, per its own doc comment. Don't generalize `CsmProjectDetailPage.tsx`'s pattern onto any of these; a genuinely-fixed-destination or genuinely-URL-driven Back link is fine to leave as-is. If a future caller ever links directly into one of the first three routes (bypassing its current single list page), revisit whether it still qualifies as fixed-destination.
