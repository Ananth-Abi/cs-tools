...
A signed-in caller whose `/users/me` call still comes back 401/403 after `useAuthApiClient`'s own retry/silent-refresh/redirect-to-sign-in chain has had its chance (see that hook's own doc comments) is treated as "not entitled to this portal yet", not a transient auth hiccup — `useAuthApiClient` already exhausts every recoverable path for 401 before an error is allowed to reach `CurrentUserContext`, so anything that does reach it is real. `AuthorizedAppShell` renders this state through `AppLayout` itself (not a bare `ErrorLayout`) with a new `minimalHeader` prop, so the real header/branding still shows — this is a signed-in user, just one without access, not someone who's left the portal. `minimalHeader` widens `Header`'s existing `hideProjectControls` flag to also hide `QuickNav`/`PinnedTabs`/pin-and-recent-views (nothing to search/pin/revisit without real pages) while still showing the theme toggle and the user's own profile avatar — and adds a `flexGrow` spacer in `Header.tsx` so `Actions` stays right-aligned once the middle elements it used to lean on are gone. The sidebar is suppressed via the existing `ErrorPageContext` flag (`setIsErrorPageDisplayed`), the same one `AppLayout` already checks for any other full-page state. `NoPortalAccessPage.tsx` is the content shown in this state — deliberately not styled as an "error" (no illustration, no sign-out link): non-technical copy, a plain muted icon.
...
Every paginated data table backed by a real MUI `<Table>` (`ProductVulnerabilitiesTab.tsx`, `DirectoryEntityTable.tsx`, `ConversationsTab.tsx`, `ChangeRequestsTab.tsx`, `IncidentsTab.tsx`, `ProblemsTab.tsx`) should match this shape — a new one should too:
